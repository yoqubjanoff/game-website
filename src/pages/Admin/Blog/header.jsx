...
const ActionRenderer = ({ data }) => {
  ...
  const getAdmins = async () => {
    try {
      const res = await request.get("/admin/blogs");
      dispatch({
        type: "setBlogs",
  ...
  const deleteF = async () => {
    try {
      const res = await request.delete(`/admin/blogs/${data?.id}`);
      getAdmins();
      Toast({
        type: "success",
  ...
  const editFunc = async () => {
    const res = await request.get(`admin/blogs/${data.id}`);
    dispatch({
      type: "setSelected",
      payload: res?.data?.data,
    });
...
const RendererDesc = ({ data }) => {
    return <Wrapper.Flex>{data?.contentUz?.slice(0,30)}...</Wrapper.Flex>;
...
const RendererStatus = ({ data }) => {
  ...
  const statusChange = async (v) => {
    console.log(v);
    try {
      const res = await request.put(`/admin/blogs`, {data:{
          id: data.id,
          isActive: v,
      }});
